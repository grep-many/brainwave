...
const Notification = ({ className, title }) => {
  return (
    <div
      className={`${
        className || ""
      } bg-n-9/40 border-n-1/10 flex animate-[bounce_5s_linear_reverse_infinite] items-center gap-5 rounded-2xl border p-4 pr-6 backdrop-blur`}
    >
        ...
        <div className="flex items-center justify-between">
          <ul className="-m-0.5 flex">
            {notificationImages?.length &&
              notificationImages.map((src, index) => (
                <li
                  key={index}
                  className="border-n-12 flex h-6 w-6 overflow-hidden rounded-full border-2"
                >
                  <img src={src} className="w-full" width={20} height={20} alt={src} />
                </li>
              ))}
          </ul>
